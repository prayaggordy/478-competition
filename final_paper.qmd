...
final_preds |>
	...
	dplyr::summarize(`Truly connected` = sum(connected),
									 `Predicted connected` = dplyr::n(),
									 `False positives` = `Predicted connected` - `Truly connected`,
									 `% false positive` = scales::percent(`False positives`/`Predicted connected`,
									 																		  accuracy = 0.001)) |>
	dplyr::rename(`Models predicting connected` = total_true)
```
